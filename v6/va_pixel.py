...
class VA_Pixel:
    ...
    def getClusterOfWordsWithInRectangle(single_word_set_of_all_possible_cordinate,pxiel_matrix):
        ...
        for i in (single_word_set_of_all_possible_cordinate):
            ...
            for search_row in range(max(row-vertical_margin_to_search,0),min(row+vertical_margin_to_search,VA_Pixel.MAX_ROWS)):
                for search_col in range(col,min(col+horizontal_margin_to_search,VA_Pixel.MAX_COLS)):
    ...
    def getLocation(string_of_text,ocr_extracted_dict):
        ...
        try:
            ...
            VA_Pixel.writelog(string_of_text+"\n")
            VA_Pixel.writelog(str(container))
            VA_Pixel.writelog("- "*100)
